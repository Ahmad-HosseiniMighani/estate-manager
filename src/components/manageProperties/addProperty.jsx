...
class AddProperty extends Form {
    ...
    doSubmit = async () => {
        ...
        const tempImages = [];
        const tempAreas = [];
        for(let i=0;i<data.areas.length;i++) tempAreas.push(data.areas[i].id);
        // console.log(tempAreas)
        delete data.areas;
        try{
            for(let i=0;i<images.length;i++) if(images[i].uploadinfo !== undefined) tempImages.push(images[i].uploadinfo.id);
            if(thumbnail[0] !== undefined && thumbnail[0].uploadinfo !== undefined) data.thumbnail = thumbnail[0].uploadinfo.url;
            const res = await auth.addProperty(data,tempImages,tempAreas);
                ...
                <div className="row">
                    <React.Fragment>
                        <div className="col-lg-8 col-10 col-md-9 mx-auto mx-auto property-properties-container shadow py-4 mt-3">
                            <div className="row">
                                <h3 className="text-center mt-3 mb-5"><i className="fas fa-home"></i> افزودن ملک جدید</h3>
