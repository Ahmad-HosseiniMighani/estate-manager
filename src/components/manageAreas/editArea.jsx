...
class EditArea extends Form {
    ...
    componentDidMount() {
        ...
        this.setState({data,id:this.props.match.params.id,IsRequestDone: true,})
    }
    doSubmit = async () => {
        ...
        try{
            ...
            if(res.status_code == 404) toast.error("منتطقه مورد نظر یافت نشد.");
            else if(res.status_code !== 200) throw res.message
            else toast.success("تغییرات منتطقه مورد نظر ثبت شد.");
            setTimeout(() => {
                window.location.replace("/manage-area");
            }, 3000);
        }catch(error){
            // console.log(error)
            toast.error("تغییرات منتطقه مورد نظر ثبت نشد. لطفا ورودی های خود را چک کنید.");
        }
    }
    render() { 
        ...
        return ( 
            <section id="area-management" dir="rtl">
                {isLoggedIn && (<div className="card col-lg-8 col-10 col-md-9 mx-auto property-photo-container shadow pt-5 pb-4 px-5">
